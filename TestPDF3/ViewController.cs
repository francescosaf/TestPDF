	...
	*/
    public partial class ViewController : UIViewController
    {
        ...
        public override void ViewDidLoad()
        {
            ...
            var documentURL = NSBundle.MainBundle.GetUrlForResource("test", "pdf");
            if (documentURL != null)
            {
                ...
                foreach (var x in page.Annotations)
                {
                    System.Diagnostics.Debug.WriteLine($"Variable==== {x.FieldName} {x.Type} {x.WidgetControlType} {x.WidgetFieldType}");
                    if (x.FieldName == "NPMA-33-1-2")
                    {

                        //x.WidgetFieldType = PdfAnnotationWidgetSubtype.Text.GetConstant();
                        //x.BackgroundColor = UIColor.Blue.ColorWithAlpha(0.25f);
                        //x.Font = UIFont.SystemFontOfSize(18);
                        //x.SetValue("Jhon xxxxxxxxx", PdfAnnotationKey.Contents);
                        //x.SetValue("Jhon xxxxxxxxx", PdfAnnotationKey.WidgetValue);

                        //x.Contents = "Jhon xxxxxxxxx";
                        //page.RemoveAnnotation(x);
                        //page.AddAnnotation(x);

                        var textFieldName = new PdfAnnotation(x.Bounds, PdfAnnotationSubtype.FreeText.GetConstant(), null)
                        {
                            WidgetFieldType = PdfAnnotationSubtype.FreeText.GetConstant(),
                            Contents = "Jhon xxxxxxxxx",
                            Color = UIColor.Cyan,
                            FieldName = "NPMA-33-1-2",
                ...
                foreach (var x in page.Annotations)
                {
                    System.Diagnostics.Debug.WriteLine($"Variable=> {x.FieldName} {x.Type} {x.WidgetControlType} {x.WidgetFieldType} {x.Contents}");
                }
